...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, iou_loss: nn.Module = None):
    ...
    def forward(
        ...
        if self.iou_loss is None:
            iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, CIoU=True)
            box_loss = 1.0 - iou
        else:
            box_loss = self.iou_loss(pred_bboxes[fg_mask], target_bboxes[fg_mask]).unsqueeze(-1)
...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        iou_loss = None
        loss_config = model.yaml.get("bbox_loss")
        if loss_config is not None:
            from ultralytics.utils.slat_loss import WiseShapeIoULoss

            loss_config = dict(loss_config)
            if loss_config.pop("type") != "wise_shapeiou":
                raise ValueError("Unsupported model bbox_loss type; expected wise_shapeiou.")
            iou_loss = WiseShapeIoULoss(**loss_config)
